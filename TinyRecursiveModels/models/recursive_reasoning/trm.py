...
class TinyRecursiveReasoningModel_ACTV1_Inner(nn.Module):
    ...
    def forward(self, carry: TinyRecursiveReasoningModel_ACTV1InnerCarry, batch: Dict[str, torch.Tensor], h_cycles: Optional[int] = None) -> Tuple[TinyRecursiveReasoningModel_ACTV1InnerCarry, torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        """Forward pass with optional runtime H_cycles override for progressive training."""
        # Use runtime override or default from config
        H = h_cycles if h_cycles is not None else self.config.H_cycles
        ...
        if use_full_bptt:
            ...
            for _H_step in range(H):
                # Optimization: Hoist invariant calculation out of inner loop
                current_input_injection = z_H + input_embeddings
                for _L_step in range(self.config.L_cycles):
                    z_L = self.L_level(z_L, current_input_injection, **seq_info)
...
class TinyRecursiveReasoningModel_ACTV1(nn.Module):
    ...
    def forward(self, carry: TinyRecursiveReasoningModel_ACTV1Carry, batch: Dict[str, torch.Tensor], 
                halt_max_steps: Optional[int] = None, h_cycles: Optional[int] = None) -> Tuple[TinyRecursiveReasoningModel_ACTV1Carry, Dict[str, torch.Tensor]]:
        """
        Forward pass with optional runtime overrides for progressive training.
        
        Args:
            carry: Current ACT carry state
            batch: Input batch dict
            halt_max_steps: Override for max ACT steps (None = use config default)
            h_cycles: Override for H-cycles in inner model (None = use config default)
        """
        # Use runtime overrides or config defaults
        T = halt_max_steps if halt_max_steps is not None else self.config.halt_max_steps
        ...
        with torch.no_grad():
            ...
            is_last_step = new_steps >= T  # Use runtime T instead of config
            ...
            if self.training and (T > 1):
                ...
                # Exploration - use runtime T for exploration range
                min_halt_steps = (torch.rand_like(q_halt_logits) < self.config.halt_exploration_prob) * torch.randint_like(new_steps, low=2, high=T + 1)
