...
Central world container managing all simulation state.
Phase 2: Health system, task assignment, institutional buildings.
"""
...
class World:
    ...
    def initialize(self, seed: Optional[int] = None):
        ...
        # Initial resource nodes (Inlined to avoid AttributeError)
        self.resource_nodes = []
    ...
    def tick_simulation(self) -> bool:
        ...
        # Migration (New settlers if population low)
    ...
    def _process_reproduction(self):
        ...
        adults = [v for v in self.villagers if 18 <= v.age <= 60 and v.health > 40 and not v.is_ill]
        if len(adults) < 2:
            return
    ...
    def _process_migration(self):
        """Allow new settlers to join if population is critically low but stable."""
        # Only migrate if population is low
        if len(self.villagers) >= 10:
            return
            
        # Must have food and housing
        if self.resources.food < 200:
            return
        if self.resources.housing_capacity <= len(self.villagers):
            return
            
        # Random chance based on stability (approx 1 every 500 ticks if stable)
        stability = calculate_village_stability(self)
        if stability < 0.4:
            return
            
        migration_chance = 0.002 * stability
        if self._rng.random() < migration_chance:
            # Create a new adult villager
            villager = create_villager(
                self._next_villager_id,
                self.width,
                self.height,
            ...
            # Make them an adult
            villager.age = random.uniform(18, 35)
            # Give them a random role
            from simulation.villager import Role
            villager.role = random.choice([Role.WORKER, Role.BUILDER, Role.FARMER])
            
            self._next_villager_id += 1
            self.villagers.append(villager)
            self.pending_messages.append("A traveler has joined the village.")
